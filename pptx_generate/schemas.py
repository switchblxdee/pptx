...
class DigestStyle(BaseModel):
    ...
    object_color: Optional[str] = Field(
        default=None,
        description=(
            "HEX-цвет (например '7B61FF') для перекраски фоновых объектов слайда-"
            "обзора: шапки групп, пилюли-источники, KPI-карточки, плашки "
            "комментариев. None — стандартные тона темы."
        ),
    )
