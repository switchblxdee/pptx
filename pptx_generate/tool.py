...
class GenerateDigestInput(BaseModel):
    ...
    style_prompt: str = Field(
        description=(
            "Текст пожеланий пользователя по стилю/оформлению. Слова про общую "
            "тему ('тёмная', 'яркая', 'корпоративная', 'пастельная') управляют "
            "палитрой — передавай их как есть, не перефразируй. "
            "НО конкретный цвет/тон ('в фиолетовых тонах', 'сделай синей') для "
            "слайда-обзора кладётся в object_color, а не сюда. Если сомневаешься "
            "— продублируй цвет и сюда, и в object_color: инструмент сам "
            "извлечёт цвет из любого места."
        )
    )
    ...
    object_color: Optional[str] = Field(
        default=None,
        description=(
            "Акцентный цвет слайда-обзора: им красятся шапки групп, пилюли-"
            "источники, KPI-карточки, рамки. HEX ('7B61FF') или название цвета "
            "рус/англ ('фиолетовый', 'purple', 'красный'). "
            "ЗАПОЛНЯЙ ВСЕГДА, когда пользователь упомянул цвет или тон для "
            "презентации — 'в фиолетовых тонах', 'фиолетовая презентация', "
            "'сделай синей', 'фоновые объекты зелёными'. Цвет фона обзора "
            "трогать НЕ нужно — он зафиксирован; этот параметр меняет только "
            "цветные объекты, а контраст текста подбирается автоматически "
            "(на тёмном объекте — белый текст). Нейтральные (чёрный/белый) сюда "
            "не клади."
        ),
    )
...
class GenerateDigestTool(BaseTool):
    ...
    description: str = (
        "Создаёт корпоративный дайджест в PowerPoint (.pptx) из xlsx-файла. "
        "РЕЖИМ ОПРЕДЕЛЯЕТСЯ АВТОМАТИЧЕСКИ по листам файла:\n"
        "• листы 'исх' + 'динамика' → плотный слайд-обзор: KPI, источники, "
        "темы по продуктам с динамикой % и статусом. Контент считается ИЗ "
        "ДАННЫХ (LLM не нужен).\n"
        "• листы 'Сырье' + 'Суммаризация' → классический дайджест (обложка + "
        "детальные слайды тем), контент генерит LLM.\n"
        "Никаких конкретных листов заранее требовать НЕ нужно — инструмент сам "
        "разберётся. Стиль (палитра, шрифты, бренд SberF1) задаётся через "
        "style_prompt; для overview можно передать period и issue_number. "
        "Возвращает путь к .pptx и отчёт по разбору данных."
    )
    ...
    def _generate_overview(
        ...
        # цвет фоновых объектов: явный параметр > «фоновые объекты X» >
        # ЛЮБОЙ цвет из промпта (для overview это единственный осмысленный
        # цветовой рычаг — фон обзора зафиксирован отдельно). Так агент может
        # сказать «в фиолетовых тонах», и карточки станут фиолетовыми, а не
        # уедут в палитру/тему.
        from .colors import (resolve_color_name, extract_object_color,
                             extract_accent_color)
        obj_hex = (resolve_color_name(object_color)
                   or extract_object_color(style_prompt)
                   or extract_accent_color(style_prompt))
        if obj_hex:
            try:
    ...
    def _generate(
        ...
        # Пояс безопасности: если кто-то вызвал стандартный путь напрямую,
        # а файл — формата overview ('исх'+'динамика'), переключаемся на него,
        # вместо падения на поиске 'Сырье'/'Суммаризация'.
        if self._looks_like_overview(xlsx_path):
            logger.info("Файл выглядит как overview ('исх'+'динамика') — режим обзора")
            return self._generate_overview(
                xlsx_path, style_prompt, output_path, force_theme, None, None, None,
            )
